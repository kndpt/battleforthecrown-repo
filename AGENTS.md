...
Hook `pre-push` (husky) léger : `yarn static-check` + unit backend + unit pixi (~10-15 s), skip si le push ne touche pas de `.ts`/`.tsx`. Les smokes sont portés par le skill `/run` en fin de tâche, pas par le hook. Détail, motivation et bypass : [`docs/architecture/local-ci.md`](./docs/architecture/local-ci.md).
