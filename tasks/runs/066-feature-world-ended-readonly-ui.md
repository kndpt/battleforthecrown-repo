...
- [ ] Backend expose `GET /worlds/:worldId/rankings/final` qui retourne les snapshots par signal (POWER / ASSAULT_GLORY / RAMPART_GLORY) avec rang, score, userId, displayName. 200 sur monde ENDED, 404/409 sur monde non-ENDED (pas de snapshot). _(auto : test backend + smoke)_
...
- T2 — Backend endpoint : `RankingsController.GET /worlds/:worldId/rankings/final` + `RankingsService.getFinalRankings(worldId)` qui lit `WorldFinalRankingSnapshot` (créée par run 061), retourne `{ leaderboards: [...] }` aligné sur le DTO de `getRankingsSummary` mais sourcé snapshot. 404 si aucun snapshot.
...
- **Consultation publique du Hall of fame** : la spec 19 § Questions ouvertes acte « Consultable indéfiniment depuis la fiche profil global, ou seulement pendant la phase `ENDED` ? Trancher au refinement ». Position par défaut MVP : consultable pendant les 7j ENDED, plus accessible une fois ARCHIVED (cohérent avec « plus accessible depuis l'UI » de la spec). À valider user étape 3.
