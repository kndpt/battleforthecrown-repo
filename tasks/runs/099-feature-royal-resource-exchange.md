...
- **Arbitrage user au démarrage** (2 décisions design, cf. § Pistes) : surface (action HUD vs bâtiment `MARKET`) + valeurs taux/plafond.

## Pistes (à trancher en étape 1 du run — ne pas inventer de chiffres avant validation)

- **Surface** : (A) action HUD dans l'écran ressources, **sans bâtiment** — reco MVP-léger, cohérent avec le « pas de bâtiment dédié » du devoir royal, évite le gating artificiel. (B) nouveau bâtiment `MARKET` — absent du catalogue `building-types.ts`, impliquerait niveaux / déblocage / coûts de construction. Le lab ticket évoque « via bâtiment Marché » pour la variante **P2P**, pas pour l'échange royal → reco (A).
...
- **Backend** :
  - `ConvertResourcesUseCase` dans `battleforthecrown-backend/src/modules/gameplay/` (calqué sur `initiate-extraction.use-case.ts` : `$transaction` isolation Serializable → `updateProduction` pour un stock à jour → débit source / crédit destination clampé → `outbox` `resources.changed` dans la même tx).
...
- **Frontend** : composant modal/sheet de conversion dans `battleforthecrown-pixi/src/features/resources/` (`ResourceBar.tsx`, `useDisplayResources.ts`), mutation TanStack Query + invalidation sur `resources.changed` WS.
...
- **T3** — `ConvertResourcesUseCase` (gameplay) : tx Serializable, `updateProduction`, débit source / crédit dest clampé, contrôle plafond + reset dayKey, Outbox `resources.changed`.
- **T4** — Endpoint `POST /resources/:villageId/convert` + DTO Zod + ownership + wiring `gameplay.module.ts`.
...
- [ ] Une conversion dépassant le plafond quotidien restant est refusée (4xx), stock inchangé — _auto (smoke)_.
- [ ] Le plafond consommé se réinitialise au passage 04:00 Europe/Paris (`dayKey`) — _auto (test)_.
- [ ] Crédit destination clampé à `maxPerType` (pas de dépassement du cap Entrepôt) — _auto (test)_.
- [ ] Event WS `resources.changed` émis après conversion — _auto (smoke)_.
- [ ] Aucune couronne convertible : l'API n'accepte que `WOOD`/`STONE`/`IRON` (Zod rejette le reste) — _auto (test)_.
- [ ] Le payload cible un seul `villageId`, sans champ village/joueur destination → pas de transfert — _auto (test)_.
