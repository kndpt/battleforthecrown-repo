...
- [ ] **[backend]** DTO/Zod `AttackCommandDto` : `posture` optionnel (default `STANDARD` à la validation). `combat.service.ts:initiateAttack` persiste la posture sur l'`Expedition`. Rejet (400) d'une posture inconnue. **Le guard lifecycle existant `assertWorldWritable(worldId, tx)` (`combat.service.ts:101`) reste en tête de `initiateAttack`** : la posture n'ajoute aucun nouveau chemin de mutation et ne contourne pas la lecture-seule monde.
...
- [ ] **[tests][régression lifecycle]** Une attaque **avec posture** reste refusée sur un monde `ENDED`/`ARCHIVED` avec le même comportement qu'aujourd'hui (`assertWorldWritable` → erreur lecture-seule), preuve que la posture n'introduit aucune mutation combat en lecture seule. Smoke `combat` ou réutilisation d'un smoke lifecycle existant (cf. `docs/gameplay/19-world-lifecycle.md` + run 065/066). _(Hors scope : le flux d'inscription `LOCKED`/`JoinWorldUseCase` n'est pas touché par ce run — la posture ne crée pas de chemin d'entrée monde.)_
