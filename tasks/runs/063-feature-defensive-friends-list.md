...
- [ ] **[SQL]** Migration Prisma crée le modèle `Friendship` avec au minimum : `id`, `worldId`, `requesterUserId`, `recipientUserId`, `status` (`PENDING` | `ACTIVE`), `createdAt`, `acceptedAt?`. Unique `(worldId, requesterUserId, recipientUserId)` + check applicatif `requesterUserId != recipientUserId`. Index `(worldId, recipientUserId, status)`.
- [ ] **[SQL/curl]** Endpoint `POST /worlds/:worldId/friendships` (body : `{ recipientUserId | recipientDisplayName }`) crée une `Friendship` `PENDING` côté demandeur. Idempotent côté demandeur (re-POST = 200 même state). Refusé 409 si une `Friendship` `ACTIVE` ou symétrique `PENDING` existe déjà.
- [ ] **[curl]** Endpoint `POST /worlds/:worldId/friendships/:id/accept` bascule en `ACTIVE` uniquement si caller = `recipientUserId` et status `PENDING`. Set `acceptedAt`. 403 sinon.
