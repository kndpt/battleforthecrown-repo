...
- [ ] Worker `RankingsCycleWorker` (pg-boss singleton, tick horaire) ferme tous les cycles hebdo échus pour tous les mondes OPEN/LOCKED : pour chaque (`worldId`, `signal`), détecte le cycle dont `cycleEndAt <= now` et qui n'a pas de `GloryCycleSnapshot`, le crée en une transaction Prisma (lecture du leaderboard sur la fenêtre `[cycleStartAt, cycleEndAt)`, écriture du snapshot, attribution du `RankingCycleTitleAward` au rang 1 avec `score > 0`).
- [ ] **Frontière de cycle** : alignée **Lundi 00:00 UTC** par défaut (configurable via `WorldConfig.rankings.weeklyCycleResetDayUtc = 1` + `weeklyCycleResetHourUtc = 0` dans `packages/shared/src/world/schemas.ts`). Le `cycleIndex 1` démarre au premier Lundi 00:00 UTC après `world.createdAt` ; pré-cycle (entre createdAt et premier lundi) ne donne pas lieu à snapshot.
...
- [ ] Smoke backend : fast-forward 2 semaines sur un monde avec scoring PvP fictif, assert 2 `GloryCycleSnapshot` créés (1 par semaine × 2 signaux = 4 lignes), assert `RankingCycleTitleAward` créé pour les top 1 par semaine, endpoint `/users/me/ranking-titles` retourne 200 + entrées attendues, endpoint `/worlds/:worldId/rankings/cycles/current` retourne le cycle en cours.
