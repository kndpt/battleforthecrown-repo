...
- **Type** : feature
- **Modules** :
  - backend : **`modules/gameplay/relocate-village.use-case.ts`** — nouveau Application Service, **frontière ADR-12** (les mutations transverses vivent dans `modules/gameplay/`, cf. `initiate-extraction.use-case.ts` / `recruit-noble.use-case.ts`, pas dans `modules/village/`). Il réutilise depuis `modules/village/` **uniquement** les services de domaine + placement. Dépendances : `modules/crowns/crowns.service.ts` (débit + `createCrownsChangedEvent`, pattern à réutiliser), `modules/world/village-placement.service.ts` (sélection tile libre, `zoneCapacity`, unique `worldId_x_y`), `modules/strategy/village-strategy.service.ts` (**référence** du pattern débit atomique + cooldown + Outbox — à copier, pas modifier), vision/world-entities (recompute disque + blip), `event/` Outbox via `OutboxPublisher(tx?)`. **Event `village.moved` OBLIGATOIRE** (pas d'alternative via `crowns.changed`/`resources.changed`) — payload figé `{ worldId, villageId, fromX, fromY, toX, toY }`.
  - frontend : `battleforthecrown-pixi/` panneau village / sélecteur multi-village (surface déclencheur) + modale confirmation (coût 200 + **avertissement changement de trait**) + éventuelle sélection de tile destination sur la carte + invalidations TanStack (`villages`/`resources`/`crowns`) + **binding WS `village.moved`** qui réconcilie la carte : purge du blip + disque de vision à l'ancienne position `(fromX,fromY)` et re-render à la nouvelle `(toX,toY)` (invalidation de remplacement, pas d'ajout sans retrait).
...
- [ ] **[auto: test/curl]** **Autorité serveur (ADR)** : le use-case rejette (403/404, jamais côté UI seule) une relocation demandée par un non-propriétaire du village, ou ciblant un village barbare (`userId = null`) ou un village d'un **autre `worldId`** que celui du demandeur. Test serveur couvrant les 3 cas.
- [ ] **[auto: outbox/smoke]** **Atomicité ADR-02** : débit couronnes + update `(x,y)` + recompute `naturalTrait` + réconciliation `zoneCapacity` + écritures Outbox `crowns.changed` / `resources.changed` (nouvelles prod rates) / `village.moved` se font dans **une seule transaction Prisma** (`OutboxPublisher(tx)`). Test de **rollback** : un échec en cours annule tout (ni village déplacé, ni event fantôme) ; `OutboxWorker` ne publie qu'**après** commit (`dispatched_at` non NULL seulement post-commit).
- [ ] **[auto: outbox]** `village.moved` émis avec payload `{ worldId, villageId, fromX, fromY, toX, toY }`.
...
- [ ] **[auto: test concurrent]** Deux `POST` relocate simultanés sur le **même** village n'aboutissent qu'à **une seule** relocation réussie et **un seul** débit de 200 couronnes (pas de double débit ni d'écrasement de position). Verrou de ligne (`SELECT … FOR UPDATE`) ou transaction `SERIALIZABLE` couvrant validation + débit + move.
...
- **T2 — [back]** `relocate-village.use-case.ts` (**dans `modules/gameplay/`**, ADR-12) : autorité serveur (ownership + `worldId`, exclut barbares), garde-fous, **sélection de tile libre SANS effet de bord** (extraire une variante read-only de `VillagePlacementService.findVillagePosition` — la version actuelle incrémente `zoneCapacity` via son propre `PrismaService`, à ne pas réutiliser telle quelle), orchestration d'une **unique transaction Prisma** couvrant tout, avec **verrou de ligne (`SELECT … FOR UPDATE`) ou niveau `SERIALIZABLE`** sur le village + gestion des conflits.
- **T3 — [back]** Débit 200 couronnes atomique dans la même `tx` + `createCrownsChangedEvent` (pattern strategy service, `WHERE balance >= cost`).
- **T4 — [back]** Recompute+persist `naturalTrait` via `deriveNaturalTrait`, update `(x,y)`, émettre `resources.changed` (nouvelles rates) + event **`village.moved`** (payload `{worldId, villageId, fromX, fromY, toX, toY}`) — tout via `OutboxPublisher(tx)` dans la même transaction.
- **T5 — [back]** Carte/vision : recompute disque de vision, mise à jour world-entities/blip, **réconciliation `zoneCapacity` via le client `tx`** — décrémenter l'ancienne zone **et** incrémenter la nouvelle dans la même transaction (jamais de mutation par un `PrismaService` indépendant avant commit).
...
- **`findVillagePosition` a un effet de bord** : la méthode actuelle de `VillagePlacementService` incrémente `zoneCapacity` via son propre `PrismaService` dès qu'une position est trouvée → l'appeler tel quel muterait le compteur même si la relocation échoue. Extraire une sélection read-only + réconcilier les deux zones dans la `tx` (cf. T2/T5).
- **Concurrence** : `updateMany WHERE balance >= 200` empêche le solde négatif mais **pas** deux relocations concurrentes du même village (double débit + écrasement de position) → verrou/`SERIALIZABLE` obligatoire (cf. critère concurrent).
