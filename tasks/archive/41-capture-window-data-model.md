...
**Statut** : ✅ Résolu 2026-05-11 par `$bftc-run @tasks/41-capture-window-data-model.md` (issue de [run 006](../runs/archive/006-audit-conquest.md))
...
4. Interaction `BarbarianSeedingCatchupWorker` : quand la cible est un village barbare, le seeding catchup ne doit se déclencher qu'**après** `village.capture-window-completed` (pas pendant la fenêtre, sinon on respawn une cible déjà engagée). À câbler côté #42 ou ajouter un consumer ici. _(note 2026-05-10, run 007) : ce worker est un catchup d'arrivée et **ne reseed pas après destruction/conquête** — la préoccupation initiale de ce point est sans objet pour le worker actuel ; à reprendre si on ouvre un vrai reseed post-conquête._
