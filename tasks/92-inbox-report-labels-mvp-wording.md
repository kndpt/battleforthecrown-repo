...
- **Piste A — aligner le code sur la spec (défaut recommandé)** : rendre les 4 labels spec sur leurs surfaces — renfort `Arrivé en soutien` / `Retour au village`, caravane `Livraison arrivée` (ARRIVED) / `Caravane rappelée` (RETURNED). Trancher au passage comment réconcilier les sous-états contextuels caravane (`Livraison réussie`/`partielle`, `Retour complet`/`partiel`) avec la catégorie unique de la spec. Rétablit le contrat livré par le run 044, la spec reste la source de vérité, aucune backprop doc. Modif ≤ 2 fichiers de vue + 2 fichiers de test.
