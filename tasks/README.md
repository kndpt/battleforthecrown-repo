...
- [078 — Modal carrousel de perte de village + notif temps réel](./runs/archive/078-feature-defeat-conquest-modal-carousel.md) — ✅ `DONE` (2026-06-20). Modal défaite carrousel côté victime sur `village.conquered`, payload + report `captureFinalized` enrichis (pseudo conquérant + snapshot castle level), acquittement PISTE B (`readByDefender` + hydratation boot). Review BLOCK→résolu (champs wire optionnels), smoke conquest 3/3.
