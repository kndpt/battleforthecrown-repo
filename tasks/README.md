...
- [078 — Modal carrousel de perte de village + notif temps réel](./runs/078-feature-defeat-conquest-modal-carousel.md) — 📋 `PLANNED`. Notifier la victime en temps réel quand un village est conquis : modal carrousel dynamique (ajout à chaud multi-villages, acquittement serveur via `readByDefender`), asset + nom du village + pseudo conquérant + CTA map. Payload `village.conquered` enrichi (pseudo + snapshot asset). Réutilise runs 024/047/048.
