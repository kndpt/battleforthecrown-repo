...
- [069 — Avertissement phase OPEN late + proposition d'un monde frais](./runs/069-feature-worlds-selection-late-phase-warning.md) — 📋 `PLANNED`. Spec 19 § Bascule cohorte principale → retardataires — bandeau « Lancé il y a {N} j » + CTA secondaire « Rejoindre plutôt un monde plus frais » sur la card `WorldsSelectionDesign`, helpers purs shared `formatWorldLaunchAgeFr` + `pickFreshAlternativeWorld`, viewmodel `worldsViewModel` étendu. UI uniquement, aucune migration DB.
...
- [066 — Écran « Monde terminé » : UI lecture seule + consultation leaderboard final](./runs/066-feature-world-ended-readonly-ui.md) — 📋 `PLANNED`. Phase 11 — UI consommatrice du snapshot livré par run 061 : `EndedWorldView` + `FinalRankingsScreen` (3 leaderboards Or/Argent/Bronze), `ctaFor` gère `ENDED`, helper `isWorldReadOnly` grise les CTAs HUD mutants, endpoint `GET /worlds/:id/rankings/final`. Dépend du run 061.
