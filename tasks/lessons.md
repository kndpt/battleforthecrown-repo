...
- Pour une bottom sheet dont la hauteur ne doit pas dépendre du contenu, définir une hauteur explicite sur l'instance du panel en plus du `maxHeight` du shell ; `maxHeight` seul laisse le contenu auto-dimensionner la sheet.
- Si une action depuis une bottom sheet ouvre une modale métier voulue par-dessus, ne pas la signaler comme régression d'empilement sans vérifier l'intention produit.
...
- Quand un champ `details` Json (legacy/tiers) est réémis vers le client via un presenter, ne jamais le passer brut : le normaliser par le codec strict en try/catch et l'omettre sur échec. Un `.parse()` client (Zod) casse sinon sur une valeur corrompue → surface entière illisible. Le crash « réglé côté serveur » se déplace côté client si on oublie la sortie.
- Pour flouter une info stratégique (précision de scout par nombre d'unités), appliquer le flou au read (presenter) en gardant l'exact en DB, et propager le MÊME palier à tout canal dérivé (carnet d'intel) : sinon le canal secondaire contourne la mécanique. Dériver le palier d'une donnée déjà persistée (compte d'espions) évite toute migration et floute les anciens rapports de façon cohérente.
- Après modification d'un composant partagé (libellé, `aria-label`), lancer la suite complète (`yarn test:pixi`) + `yarn static-check`, jamais un vitest ciblé sur le seul fichier du run : le test ciblé ne voit ni les assertions des fichiers voisins hors diff (`ToastStack.test.tsx` asserte les aria-labels de `ToastStack.tsx`), ni les erreurs de type — le typage étant effacé au runtime, un `UnitType` inexistant (`SPEARMAN`) passe vert en vitest et ne casse qu'au `tsc`.
