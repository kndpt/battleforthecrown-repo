...
| `attack.incoming` | `userId` (défenseur) | `expeditionId, targetVillageId, targetX, targetY, arrivalAt` | `CombatService.initiateAttack` (émis dans la même tx que `battle.sent`, **uniquement** si `targetKind = PLAYER_VILLAGE`). Routé via `userByVillage('targetVillageId')` → propriétaire du village ciblé ; un village barbare (`userId = null`) n'a aucun destinataire. **Fog-of-war safe par construction** : ni composition d'armée, ni identité/origine de l'attaquant. Le frontend invalide `combat/incoming/:villageId`. Cf. [`docs/gameplay/16-notifications.md`](../gameplay/16-notifications.md). |
...
| `village.capture-window-opened` | `userId` (attaquant **+ défenseur** propriétaire original) | `pendingConquestId, targetVillageId, attackerVillageId, captureUntil` | `ConquestService.openCaptureWindow`. Dual-route (run 094) : `planCaptureWindowOpened` route l'attaquant (copie full) **et** le propriétaire original du village cible, résolu à la volée via `getUserIdByVillage(targetVillageId)` (pendant `OPEN`, `Village.userId` est encore le défenseur). La **copie défenseur est fog-scrub** (`scrubAttackerFields` retire `attackerUserId`/`attackerVillageId`). Cible barbare (`userId = null`) → jamais routée au défenseur. |
| `village.capture-window-completed` | `userId` (nouveau propriétaire **+ `previousOwnerUserId`**) | `pendingConquestId, targetVillageId, newOwnerUserId, previousOwnerUserId?` | `ConquestFinalizeWorker` via `ConquestService.finalizeCaptureWindow`. `previousOwnerUserId` = **snapshot** de `conquerVillageInTx.previousOwnerId` (jamais résolu live : à ce stade `targetVillageId` pointe déjà sur le nouvel occupant). `planCaptureWindowCompleted` route les deux (skip si identiques). Absent pour une cible sans propriétaire (barbare). |
| `village.capture-window-interrupted` | `userId` (attaquant **+ défenseur** propriétaire original) | `pendingConquestId, targetVillageId, reason` | `ConquestService.interruptCaptureWindow`. Même dual-route fog-scrub que `capture-window-opened` (run 094). |
...
| `village.conquered` | `userId` (nouveau propriétaire + ancien propriétaire si PvP) | `villageId, villageName, newOwnerId, newOwnerName, previousOwnerId, previousTier, lostVillageVisualTier, x, y, buildingsKept` | `ConquestService` (`newOwnerName` = pseudo conquérant ; `lostVillageVisualTier` = tier visuel 1-6 du village perdu, snapshot au moment T — alimente la modal défaite carrousel côté victime) |
| `rankings.cycle.closed` | `worldId` | `worldId, signal, cycleIndex, cycleEndAt` | `RankingsCycleService.closeDueCycles` (worker `RankingsCycleWorker`, tick horaire) à la clôture d'un cycle hebdo Gloire (run 068). Front : invalide `rankings.summary`, `rankings.cycles` et `ranking-titles`. |
| `world.status.changed` | `worldId` | `worldId, from, to, at` | `WorldLifecycleWorker` quand un monde passe `PLANNED → OPEN`, `OPEN → LOCKED`, `LOCKED → ENDED` ou `ENDED → ARCHIVED` (run 065, émis dans la même transaction que la purge des données joueur) |
| `world.planned.created` | `worldId` | `worldId, plannedOpenAt, source` (`'auto'`) | `WorldLifecycleWorker.ensurePlannedPipeline` quand le spawner auto-crée un monde `PLANNED` pour garantir un monde joignable « frais » (cadence `newWorldEverydays`). Broadcast `directWorld`, mais le monde fraîchement créé n'a aucun membre → aucun client ne le reçoit (binding front no-op). Trace Outbox pour de futurs consommateurs. La création **manuelle** (seed/admin) n'émet pas cet event. |
| `world.inscription-phase.changed` | `worldId` | `worldId, from` (`'main'`)`, to` (`'late'`)`, at` | `WorldLifecycleWorker.handlePhaseTransitions` quand un monde `OPEN` traverse `startedAt + inscriptionMainDays` (cohorte principale complète → inscription retardataires). Le `WorldStatus` reste `OPEN` — c'est un sous-flag persisté (`World.inscriptionPhaseTransitionedAt`) garantissant l'idempotence (émis une seule fois). Front : invalide `worlds.public` pour faire apparaître le bandeau « Lancé il y a {N} j » sans refetch manuel. Cf. [`docs/gameplay/19-world-lifecycle.md`](../gameplay/19-world-lifecycle.md). |
| `pvp.shield.broken` | `userId` (joueur dont le bouclier casse) | `userId, worldId, brokenAt, endsAt` | `NewbieShieldService.breakAttackerShieldIfActive` (via `CombatService.initiateAttack`) quand un joueur sous bouclier débutant lance une attaque PvP sortante. Le frontend invalide `memberships` + `world-entities`. Cf. [`docs/gameplay/14-pvp-conquest.md` §3](../gameplay/14-pvp-conquest.md). |
| `extraction.started` | `userId` (propriétaire du village) | `expeditionId, worldId, villageId, siteId, resourceType, arrivalAt, durationMs` | `InitiateExtractionUseCase.execute` au dispatch de l'équipe d'extraction. Anti-leak : pas de position du site, pas de capacité restante. |
| `extraction.depleted` | `worldId` (broadcast) | `worldId, siteId, resourceType` | `ExtractionLifecycleService.depleteAndRespawnIfExhausted` quand le site s'épuise. Payload minimal, aucune info occupant. Front : invalide `world-entities`. |
| `extraction.attacked` | `userId` (propriétaire du village, l'équipe attaquée) | `expeditionId, worldId, villageId, siteId, interrupted, stolen` | `ExtractionLifecycleService.handleInterception` quand une attaque intercepte l'équipe en EXPLOITING. |
| `extraction.returned` | `userId` (propriétaire du village) | `expeditionId, worldId, villageId, resources` | `ExtractionLifecycleService.handleReturn` au retour de l'équipe d'extraction. |
...
Les cartes quotidiennes consomment ces mêmes facts côté backend : `EventOutboxService` projette les events éligibles vers `DailyCardTask` avant le dispatch WebSocket. La projection est idempotente via `DailyCardProgressEvent.eventOutboxId`, donc un retry d'Outbox ne double pas la progression.

L'onboarding scripté consomme aussi ces facts, mais dans un état dédié : `OnboardingService.recordOutboxEvent` projette seulement l'étape courante (`building.completed` Château/Caserne/Watchtower, `unit.trained`, `battle.resolved` barbare victorieux). `OnboardingProgressEvent.eventOutboxId` et l'unicité `state × step` garantissent qu'un replay d'Outbox ne double ni progression ni statut.
