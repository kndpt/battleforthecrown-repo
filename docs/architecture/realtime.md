...
| `village.conquered` | `userId` (nouveau propriétaire + ancien propriétaire si PvP) | `villageId, villageName, newOwnerId, newOwnerName?, previousOwnerId, previousTier, villageCastleLevel?, x, y, buildingsKept` (`newOwnerName`/`villageCastleLevel` = enrichissement modal défaite côté victime ; optionnels sur le wire pour décoder les rows Outbox antérieures au 2026-06-20) | `ConquestService` |
