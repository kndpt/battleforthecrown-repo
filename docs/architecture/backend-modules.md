...
| **users** | `GET /worlds/:worldId/users/:userId/public-profile` | — | Fiche publique d'un joueur tiers world-scopée (run 082), **JWT-protected**. Expose uniquement les champs publics (spec 09 § Visibilité) : `displayName` + `kingdomPower` (réutilise `PowerService.getPublicKingdomPower`) + état bouclier débutant live (`NewbieShieldService`, `null` si inactif). **404** `USER_NOT_MEMBER_OF_WORLD` si non-membre (barbare inclus). Aucune fuite renown/villages/couronnes/armée/intel |
