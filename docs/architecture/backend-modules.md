...
| **combat** | `POST /combat/attack`, `POST /combat/scout`, `POST /combat/reinforce`, `POST /combat/recall`, `POST /combat/recall/:expeditionId`, `GET /combat/:villageId/active`, `GET /combat/:villageId/incoming`, `GET /combat/:villageId/garrison`, `GET /combat/reports`, `GET /combat/scout-reports` | `CombatWorker`, `ReturnWorker` | Attaque, scout, renforts, garnison, conquête, butin, retour. `/incoming` = menaces entrantes côté défenseur (event `attack.incoming`, fog-safe, run 086). Stratégies `Barbarian` / `Player`. Renfort cross-joueur autorisé vers un **ami défensif `ACTIVE`** (`FriendshipService`), bloqué sur cible sous `PendingConquest` `OPEN` ; scout révèle `defensiveFriendsDisplayNames` (run 063) |
...
| **users** | `GET /worlds/:worldId/users/:userId/public-profile` | — | Fiche publique d'un joueur tiers world-scopée (run 082), **JWT-protected**. Expose uniquement les champs publics (spec 09 § Visibilité) : `displayName` + `kingdomPower` (réutilise `PowerService.getPublicKingdomPower`) + état bouclier débutant live (`NewbieShieldService`, `null` si inactif) + indicateur d'inactivité pré-abandon (run 087 : `inactivity` `{ state: 'INACTIVE', sinceDays }` ou `null` si actif ; dérivé en lecture seule de `lastLoginAt` via `computeInactivityState` shared, seuil 7 j ; **jamais** `lastLoginAt` brut). **404** `USER_NOT_MEMBER_OF_WORLD` si non-membre (barbare inclus). Aucune fuite renown/villages/couronnes/armée/intel |
| **friendship** | `POST /worlds/:worldId/friendships`, `POST …/:id/accept`, `DELETE …/:id`, `GET …/me` | — | Amis défensifs (run 063). Lifecycle `Friendship` `PENDING`→`ACTIVE`, cap **5 `ACTIVE`** par joueur revérifié des deux côtés à l'`accept` sous `SELECT … FOR UPDATE`. `FriendshipService` consommé par `combat` (guard renfort + scout). Spec [`20-defensive-friends.md`](../gameplay/20-defensive-friends.md) |
| **map-marker** | `GET /worlds/:worldId/map-markers`, `POST …` (upsert), `PATCH …/:id`, `DELETE …/:id` | — | Marqueurs de carte privés (run 085). CRUD scopé `userId × worldId`, posés sur tile libre `(x, y)` (unicité `(userId, worldId, x, y)`), 6 kinds + note ≤80, cap **50** revérifié dans la tx de l'insert. JWT obligatoire, ownership 404 sans fuite, `assertWorldWritable` sur les mutations (GET ok en `ENDED`). Aucun event WS (privé compte). Purgé à l'archive monde. Spec [`26-private-map-markers.md`](../gameplay/26-private-map-markers.md) |
...
Mutations gardées (appel obligatoire) : attack, scout, reinforce, caravan, train, recruit-noble, upgrade building, change strategy, claim daily card, claim onboarding completion reward, map-marker create/update/delete.
...
L'archivage (`ENDED → ARCHIVED` à `endsAt + archiveAfterDays`) est porté par `WorldLifecycleWorker.archiveEndedWorlds` : transaction unique qui purge les données joueur scopées au monde (Village + cascades, Expedition, VillageIntel, MapMarker, seed/chunk/zone state, CrownBalance, DailyCard, DailyOyez, OnboardingState) et conserve `World`, memberships, rapports, snapshot final, gloire/renown et cosmétiques permanents.
