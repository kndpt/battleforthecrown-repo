...
| `Expedition.kind` | `ATTACK`, `REINFORCE`, `SCOUT`, `CARAVAN` ou `EXTRACTION`. Détermine le comportement à l'arrivée. |
...
| `Expedition.extractionSiteId` / `extractionDurationMs` / `exploitationEndsAt` / `extractionVillagers` | champs nullables portant le cycle `kind: EXTRACTION` (run 091) : site ciblé (relation `ResourceExtractionSite`, `onDelete: SetNull`), durée choisie à l'envoi (2/4/8 h), fin de phase stationnaire `EXPLOITING` (nouveau statut `ExpeditionStatus`), nombre de villageois envoyés. `TargetKind.EXTRACTION_SITE` complète le ciblage combat pour l'interception d'une équipe en exploitation. |
...
### Sites d'exploitation de ressources

| Table | Rôle |
|-------|------|
| `ResourceExtractionSite` | site rare de ressource (Bosquet royal/bois, Carrière ancienne/pierre, Mine abandonnée/fer, run 091). Champs : `worldId`, `x`, `y` (unique `(worldId, x, y)`), `resourceType` (enum `ExtractionResourceType` : `WOOD`/`STONE`/`IRON`), `initialCapacity`/`remainingCapacity`, `state` (enum `ExtractionSiteState` : `ACTIVE`/`DEPLETED`), `depletedAt`. Index `(worldId, state)`. Relation `world` (`onDelete: Cascade`) ; relation inverse `Expedition.extractionSite` en `onDelete: SetNull` (le site peut être supprimé/respawné sans invalider l'historique d'expédition). Seedé par `ExtractionSiteSeedingService` (`floor(membres du monde × 0.5)`, toujours < nombre de joueurs) au join d'un membre, respawn ailleurs dans le monde à l'épuisement. Exposé fog-gated **inconditionnellement** (même si `fogOfWarEnabled=false`) via `GET /world/:id/entities` (`kind: RESOURCE_EXTRACTION_SITE`), payload anti-leak limité à `{resourceType, activity}` — jamais la capacité ni l'occupant. Mécanique complète : [`docs/gameplay/lab/tickets/07-resource-extraction-sites.md`](../gameplay/lab/tickets/07-resource-extraction-sites.md). Events Outbox associés : [`realtime.md`](./realtime.md). Source de vérité des champs exacts : [`prisma/schema.prisma`](../../battleforthecrown-backend/prisma/schema.prisma). |
