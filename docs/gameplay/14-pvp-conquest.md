...
| UX joueur | Sur le panneau d'info ennemi : bouton attaque grisé + message *« Puissance trop faible »*. |
...
> **Implémentation (MVP)** : seuil dérivé par le helper pur `isAttackAllowedByPowerRatio` (constante unique `POWER_RATIO_DIVISOR = 3`, `packages/shared/src/pvp/power-ratio.ts`) — comparaison entière `defenderPower × 3 ≥ attackerPower` (pas de division flottante). Garde serveur dans `CombatService.initiateAttack` (403 `POWER_RATIO_FORBIDDEN`), évaluée au lancement uniquement, sur la **puissance du propriétaire** de la cible (`target.userId`). Périmètre `PLAYER_VILLAGE` uniquement (barbares et scout exclus). Puissance par joueur réutilisée depuis le snapshot déjà calculé pour la résolution (relecture seulement si le propriétaire n'y figure pas, ex. fenêtre de capture ouverte). Côté front : pré-check UX dans `SelectedEntityPanel` (CTA grisé + message *« Puissance trop faible »* via la puissance publique `GET /power/kingdom/:userId/public`) et interception du 403 dans `AttackDetailModal` (`combatErrorMessage`). Le serveur reste autoritatif.
