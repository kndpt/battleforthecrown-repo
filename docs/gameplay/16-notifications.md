...
3 catégories de notifications, chaînées au pattern Outbox côté backend (cf. [`docs/architecture/realtime.md`](../architecture/realtime.md)).
...
Détails à trancher plus tard : opt-in/opt-out par catégorie, regroupement (un seul push pour 3 entraînements simultanés ?), latence acceptable, fallback in-app (badge HUD) si push refusé par l'OS, intégration FCM/APNs et plomberie côté backend.
