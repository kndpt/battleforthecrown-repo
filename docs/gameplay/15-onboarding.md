...
2. Acquitte et masque le tutoriel : `OnboardingState.completionRewardApplied`, exposé dans `OnboardingSummaryDto`, est l'**acquittement durable** — `getOnboardingGuidance` masque définitivement la guidance « completion » dès qu'il est `true`, sur tout appareil et toute session. Le `sessionStorage` (clé `userId × worldId`, `onboardingCompletion.ts`) n'est qu'un cache optimiste couvrant la fenêtre entre le clic et le refetch du summary : seul, il meurt avec l'onglet et l'écran de fin se ré-afficherait à chaque relance. Le front invalide `resources` à la réussite du claim pour faire apparaître le butin.
