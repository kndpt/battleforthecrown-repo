...
- **Annulation** : remboursement complet du coût (ressources + population, et couronnes pour le Seigneur) des **unités restantes** — les unités déjà entraînées sont conservées. Annuler la tête de file promeut la formation suivante ; annuler une formation en attente ne perturbe pas la tête active.
