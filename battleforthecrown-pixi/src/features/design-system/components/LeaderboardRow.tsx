...
const medalClass: Record<Exclude<LeaderboardRankTone, "default">, string> = {
  ...
  silver:
    "border-[#4d595e] bg-[radial-gradient(circle_at_30%_25%,#f7fafc,#7e8b91)]",
  bronze:
    "border-[#492810] bg-[radial-gradient(circle_at_30%_25%,#f4cea3,#7a4915)]",
};

const avatarToneClass: Record<
  NonNullable<LeaderboardRowProps["avatarTone"]>,
  string
> = {
...
export function LeaderboardHeader({
  ...
  return (
    <div
      className={cn(
        "grid gap-2 px-2.5 pb-1 font-game text-[9.5px] font-bold uppercase tracking-[.14em] text-[#6d5838]",
        showDelta
          ? "grid-cols-[42px_1fr_auto_auto]"
    ...
    >
      ...
      {showDelta ? <span>{deltaLabel}</span> : null}
...
export function LeaderboardRow({
  ...
  return (
    <Tag
      className={cn(
        "grid w-full items-center gap-2 rounded-[10px] border-2 px-2.5 py-2 text-left shadow-[inset_0_1px_0_rgba(255,255,255,.45)]",
        delta ? "grid-cols-[42px_1fr_auto_auto]" : "grid-cols-[42px_1fr_auto]",
        rowToneClass[rankTone],
        self
          ? "border-[#9e7b0d] bg-[linear-gradient(to_bottom,#fef0c6,#e8c878)]"
          : "",
    ...
    >
      <div className="text-center font-game text-base font-extrabold tabular-nums text-[#3d2f1f]">
        {medalTone ? (
          <span
            className={cn(
              "inline-flex size-9 items-center justify-center rounded-full border-2 font-game text-lg font-black text-white [text-shadow:1px_1px_2px_rgba(0,0,0,.5)]",
              medalClass[medalTone],
            )}
          >
            <span className="-translate-y-px leading-none">{rank}</span>
          </span>
        ) : (
          <span className="-translate-y-px inline-block leading-none">{rank}</span>
        )}
      </div>
      <div className="flex min-w-0 items-center gap-2">
        <div
          className={cn(
            "flex size-9 shrink-0 items-center justify-center rounded-lg border-2 border-[#3d2f1f]",
        ...
        <div className="flex min-w-0 flex-col">
          <span className="truncate font-game text-[12.5px] font-bold leading-[1.1] text-[#3d2f1f]">
            {name}
          </span>
          <span className="truncate font-game text-[10px] text-[#6d5838]">
            {subtitle}
          </span>
        </div>
      </div>
      {delta ? (
        <span
          className={cn(
            "inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 font-game text-[11px] font-bold tabular-nums",
            deltaClass[delta.tone],
          )}
        >
          {delta.label}
        </span>
      ...
      <div className="min-w-20 text-right font-game text-sm font-extrabold tabular-nums text-[#3d2f1f]">
        {points}
        <small className="block text-[9.5px] font-semibold uppercase tracking-[.1em] text-[#6d5838]">
          {scoreLabel}
        </small>
      </div>
    </Tag>
