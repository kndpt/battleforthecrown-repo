...
import { Button, Input, InputHelperText, Modal, ModalBody, Spinner } from '@/ui';
import {
...
import { calculateDistance, calculateTravelTime, formatTravelTime } from '@/lib/combatHelpers';
import { CARAVAN_SPEED, CARRY_PER_PORTER } from '@battleforthecrown/shared/logic';
...
export function CaravanLaunchModal({
  ...
  const porters = totalVolume > 0 ? Math.ceil(totalVolume / CARRY_PER_PORTER) : 0;
  ...
  const submit = () => {
    ...
    caravan.mutate(
      ...
      },
      {
        onSuccess: () => onClose(),
        onError: (err) => {
          setError(err instanceof ApiError ? err.message : 'Échec de la caravane');
        },
      },
    );
  };
      ...
      <ModalBody className="!p-0 relative flex max-h-[90vh] flex-col overflow-hidden">
        ...
        <div className="flex h-32 flex-shrink-0 items-center justify-center border-b-4 border-game-gold-border bg-gradient-to-br from-game-gold-light to-game-gold-dark">
          <div className="text-center">
            <Package className="mx-auto mb-2 text-white drop-shadow" size={34} />
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          {resourcesQuery.isLoading || populationQuery.isLoading ? (
            <div className="flex justify-center py-8">
              <Spinner size="lg" />
            </div>
          ) : (
            <div className="space-y-3">
              {(['wood', 'stone', 'iron'] as const).map((key) => (
                <label
                  key={key}
                  className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded border border-kingdom-300 bg-white/60 p-3"
                >
                  <img className="size-7 object-contain" src={RESOURCE_ICONS[key]} alt="" />
                  <span className="text-sm font-semibold text-kingdom-800">
                    {RESOURCE_LABELS[key]}
                  </span>
                  <Input
                    className="w-28 text-right"
                    min={0}
                    max={stock?.[key] ?? undefined}
                    type="number"
                    value={resources[key]}
          ...
          <div className="rounded border border-game-stone-border/30 bg-game-stone-light/10 p-3 text-sm text-kingdom-800">
            <div className="flex justify-between gap-3">
              <span>Volume</span>
              <strong>{totalVolume.toLocaleString('fr-FR')}</strong>
            </div>
            <div className="mt-1 flex justify-between gap-3">
              <span>Porteurs</span>
              <strong className={hasEnoughPopulation ? '' : 'text-game-red-dark'}>
