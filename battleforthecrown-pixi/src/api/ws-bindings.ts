...
export function applyReinforcementReturned(
  ...
  if (expeditionId) {
    ...
    markReturnedReinforcementSnapshots(
      payload.villageId,
...
export function applyVillageConquered(
...
export function applyVillageRemoved(
  payload: VillageRemovedPayload,
  ctx: BindingsContext,
): void {
  ctx.queryClient.invalidateQueries({
    queryKey: queryKeys.worldEntities(payload.worldId),
...
export function applyNobleKilled(
  ...
  ctx.queryClient.invalidateQueries({
    queryKey: queryKeys.population(payload.attackerVillageId),
  });
  ctx.queryClient.invalidateQueries({
...
function invalidateConquestAttackerState(
  ctx: BindingsContext,
  attackerVillageId: string,
): void {
  ctx.queryClient.invalidateQueries({
    queryKey: queryKeys.activeExpeditions(attackerVillageId),
  });
  ctx.queryClient.invalidateQueries({
    queryKey: queryKeys.armyInventory(attackerVillageId),
  });
  ctx.queryClient.invalidateQueries({
    queryKey: queryKeys.population(attackerVillageId),
  });
}
...
function markReturnedReinforcementSnapshots(
  villageId: string,
...
function invalidateReinforcementQueries(
  ...
  const uniqueVillageIds = new Set(
    villageIds.filter((villageId): villageId is string => Boolean(villageId)),
  );
...
export function applyIntelUpdated(
  payload: IntelUpdatedPayload,
  ctx: BindingsContext,
): void {
  ctx.queryClient.invalidateQueries({
...
export function bindServerEvents(ctx: BindingsContext): () => void {
  ...
  for (const eventName of Object.keys(bindings) as ServerEventName[]) {
    const handler = bindings[eventName] as ServerEventListener<
      typeof eventName
    >;
    offs.push(
