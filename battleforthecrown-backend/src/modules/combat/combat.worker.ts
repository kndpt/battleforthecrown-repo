...
import { CaravanArrivalService } from './caravan-arrival.service';
import { ReinforcementArrivalService } from './reinforcement-arrival.service';
...
export class CombatWorker implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly scoutArrival: ScoutArrivalService,
    private readonly caravanArrival: CaravanArrivalService,
    private readonly reinforcementArrival: ReinforcementArrivalService,
  ) {}
          ...
          this.prisma.$transaction(
            async (tx): Promise<CombatPostCommitWork | undefined> => {
              ...
              if (expedition.kind === ExpeditionKind.REINFORCE) {
                await this.reinforcementArrival.handleArrival(tx, expedition);
                return;
              }

              if (expedition.kind === ExpeditionKind.SCOUT) {
                await this.scoutArrival.handleArrival(tx, expedition);
                return;
              }

              if (expedition.kind === ExpeditionKind.CARAVAN) {
                const returnJob = await this.caravanArrival.handleArrival(
                  tx,
                  expedition,
                );
                return { returnJob };
