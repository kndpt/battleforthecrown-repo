...
- status: fixed
  ...
  branch: maint/debt/worldmap-scene-cleanup
  title: "maint(debt): simplify WorldMapScene background tap and exit cleanup"
  note: >
    Grouped background/continent/decor/grid/border into mapGroundLayer; replaced 11-target
    pointertap whitelist with isMapBackgroundTap helper; exit() now destroys entity/blip
    visuals (parity with VillageScene) instead of only clearing Maps.
  verification: pixi type-check ✓ · lint ✓ · 404 pixi tests ✓
...
- 2026-06-12: WorldMapScene background tap + exit cleanup on `maint/debt/worldmap-scene-cleanup` —
  grouped non-interactive map layers into `mapGroundLayer` (eventMode static, children none) and
  replaced the 11-target pointertap whitelist with `isMapBackgroundTap` helper; `exit()` now
  destroys entity/blip visuals like VillageScene instead of only clearing the Maps.
  pixi type-check ✓ · lint ✓ · 404 pixi tests ✓.
