...
last: 2026-07-03 | theme consolidation `retention.service.ts` — merge `ensureDailyCard{,InTransaction}` en un helper unique acceptant `PrismaClientOrTx` (upsert natif, drop try/catch P2002) + merge `expireStaleCards{,InTransaction}` en drop du `$transaction` inutile (2× updateMany statuts disjoints) + commentaire V2 sur backfill post-04:00 Paris. Fichier unique, comportement inchangé, 554 unit + 6 smokes daily-retention verts.
full: `archive/refactor-backend/2026-07-03-full.md`
...
| W1  | High | combat/combat.worker.ts (2045L)                    | 4 kinds cohabitent — split par kind, L effort                                              |
| B1  | Med  | combat.service.ts                                  | 1477L sans spec unit direct (smokes uniquement)                                            |
| E1  | Med  | 12 fichiers, 46 callsites `createOutboxEvent`      | migration progressive vers `OutboxPublisher` typé (5 kinds couverts)                       |
...
- K1 + T1 + V2 (retention helpers consolidation) → ce run
